        ...
        <div class="col-xl-12 col-sm-12 col-12">
            <div class="card oth_income_card oth_library">
                <div class="card-header" style="background-color: #2f47ba;">
                    <h2 class="card-titles" style="color: white;">Picture App Audit <i style="float:right; cursor: pointer;" id="oth_library-ico" class="oth_library-ico"></i></h2>
                </div>
                                ...
                                <div class="col-md-3">
                                    <select id="emp_list" class="form-control form-select w-100">
                                        ...
                                        @foreach($tbl_employee as $emp)
                                            <option value="{{$emp->bio_id}}">{{$emp->emp_code}} - {{$emp->last_name}}, {{$emp->first_name}} {{$emp->middle_name}} {{$emp->ext_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div><br>
...
<script>
    $(document).ready(function() {
        ...
        function loadFaceTimeAuditImages() {
            var bio_id = $("#emp_list").val();
            
            $.ajax({
                url: "{{ route('load_picture_app_audit_tbl') }}",
                type: "POST",
                dataType: "json",
                data: {
                    "_token": "{{ csrf_token() }}", 
        ...
        $("#export-btn").on("click", function() {
            ...
            form.method = 'POST';
            form.action = "{{ route('export_picture_app_audit') }}";
            ...
            var bioIdInput = document.createElement('input');
            bioIdInput.type = 'hidden';
            bioIdInput.name = 'bio_id';
            bioIdInput.value = bioId;
            form.appendChild(bioIdInput);
        ...
        $("#export-entries-btn").on("click", function() {
            ...
            var form = document.createElement('form');
            form.method = 'POST';
            form.action = "{{ route('export_all_entries') }}";
