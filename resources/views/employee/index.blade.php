...
<script>
    	...
    	$( document ).ready(function() {
            ...
            $("#save_btn").on("click", function(){
                ...
                var formData = new FormData();
                  //  formData.append('emp_img', $('input[type=file]')[0].files[0]);
                    let file = $("#emp_file")[0].files[0]; // correct input

                    if (file !== undefined) {
                        formData.append("emp_img", file);
                    }
            ...
            // employee upload
            $('#uploadEmployeeForm').on('submit', function(e){
                e.preventDefault();
                
                  let fileInput = $('#employee_excel_file')[0].files;
    
				// Check if file is selected
				if (fileInput.length === 0) {
					$.notify("Please select a file before uploading.", { type: "danger", icon: "exclamation" });
                ...
                $.ajax({
                    url: "{{ route('upload.excel.employee') }}", //upload.excel.employee
                    type: "POST",
                    data: formData,
                    contentType: false,
                    processData: false,
                    success: function(response) {
                        $.notify(response.message, { type: "success", icon: "check" });
						$('#employee_excel_file').val('');
                        if (response.employee_count !== undefined) {
                            $('.employee-count-label').html(`<strong>${response.employee_count} Employee(s)</strong>`);
                        }
                    ...
                    error: function(xhr) {
                        $.notify(xhr.responseJSON?.error || "Upload failed", { type: "danger", icon: "exclamation" });
						HoldOn.close();
                    }
                });
            });
